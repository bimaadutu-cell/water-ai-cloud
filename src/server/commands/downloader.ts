...
async function validateDownloadedMedia(file: string, workDir: string, forced: "image" | "video" | "any"): Promise<{ buffer: Buffer; filename: string; mimetype: string; kind: "image" | "video" }> {
  ...
  // Always re-mux/re-encode video to WhatsApp-safe H.264 + AAC MP4 (fixes corrupt/green frames)
  if (kind === "video") {
    ...
    return {
      ...
      filename: `${sanitizeFilename(path.basename(file, path.extname(file)))}.mp4`,
      mimetype: "video/mp4",
      kind: "video",
    };
  }
  // Normalize images to JPEG for maximum WA compatibility
  if (kind === "image" && detected.mime !== "image/jpeg") {
    try {
      const sharp = (await import("sharp")).default;
      const jpeg = await sharp(original).rotate().jpeg({ quality: 92, mozjpeg: true }).toBuffer();
      if (jpeg.length >= 64) {
        return {
          buffer: jpeg,
          filename: `${sanitizeFilename(path.basename(file, path.extname(file)))}.jpg`,
          mimetype: "image/jpeg",
          kind: "image",
        };
      }
  ...
  return {
    buffer: original,
    filename: `${sanitizeFilename(path.basename(file, path.extname(file)))}.${ext}`,
...
async function downloadCommand(ctx: CmdCtx, mode: "audio" | "video"): Promise<CmdResult> {
  ...
  try {
    ...
    if (!media?.buffer || media.buffer.length < 64) {
      throw new CmdError("❌ Unduhan selesai tetapi file media kosong/rusak. Coba URL atau judul lain.");
    }
    const buf = Buffer.isBuffer(media.buffer) ? media.buffer : Buffer.from(media.buffer as any);
    if (buf.length < 64) {
      throw new CmdError("❌ Buffer media tidak valid setelah normalisasi.");
    }
    const kind = mediaKind(media.mimetype || (mode === "audio" ? "audio/mpeg" : "video/mp4"));
    const caption = mediaCaption({ ...media, buffer: buf }, mode);
    // Thumbnail opsional — thumbnail rusak sering bikin sendMessage gagal
    let thumb: Buffer | undefined;
    if (media.thumbnail && Buffer.isBuffer(media.thumbnail) && media.thumbnail.length > 32 && media.thumbnail.length < 100_000) {
    ...
    if (key) {
      await progress(
        ...
        ctx.n.remoteJid,
        key,
        `✅ Selesai (${(buf.length / 1048576).toFixed(2)} MB). Mengirim media...`
      );
    }
    return {
      // Caption cukup di media agar engine mengirim file (bukan teks saja)
      media: {
        kind,
        buffer: buf,
        filename: media.filename || (kind === "audio" ? "audio.mp3" : "video.mp4"),
        mimetype: media.mimetype || (kind === "audio" ? "audio/mpeg" : "video/mp4"),
        caption,
        jpegThumbnail: thumb,
      },
    };
...
/* ===== V3.6 UPGRADE: external MP3 endpoints + improved play flow ===== */
...
export async function playV35(ctx: CmdCtx): Promise<CmdResult> {
  ...
  async function sendAudio(media: DownloadedMedia): Promise<CmdResult> {
    if (!media?.buffer || media.buffer.length < 64) {
      throw new CmdError("❌ File audio kosong setelah unduhan. Coba judul/URL lain.");
    }
    const buf = Buffer.isBuffer(media.buffer) ? media.buffer : Buffer.from(media.buffer as any);
    const caption = mediaCaption({ ...media, buffer: buf }, "audio");
    if (key) {
      await progress(
        ctx.sock,
        ctx.n.remoteJid,
        key,
        `✅ Selesai (${(buf.length / 1048576).toFixed(2)} MB). Mengirim audio...`
      );
    }
    return {
      text: caption,
  ...
  // 3e) Multi-mirror Cobalt / Piped / Invidious (50+ real endpoints)
  try {
    if (key) await progress(ctx.sock, ctx.n.remoteJid, key, `⬇️ Mirror download (${MIRROR_COUNT} sumber)...`);
    const mirrored = await downloadAudioMulti(arg, {
      preferUrl: ytUrl,
      title: info.title,
    });
    if (mirrored) {
    ...
    errors.push("mirrors: all failed");
  } catch (e: any) {
    errors.push(`mirrors: ${String(e?.message || e).slice(0, 80)}`);
  }
  ...
  throw new CmdError(
    ...
    `Tips:\n` +
    `• Coba judul lebih spesifik atau URL Spotify/SoundCloud/YouTube\n` +
    `• Server memakai ${MIRROR_COUNT}+ mirror download real-time\n` +
    `• Opsional: set COBALT_API_URL / YTDLP_COOKIES\n` +
...
export async function play2(ctx: CmdCtx): Promise<CmdResult> {
  ...
  if (!arg) {
    return {
      text:
        `🎵 *PLAY2 — Spotify Player*\n\n` +
        `Pakai: ${ctx.bot.prefix}play2 <judul lagu>\n` +
        `Contoh: ${ctx.bot.prefix}play2 Bergema sampai selamanya\n\n` +
        `Cari di Spotify (bukan YouTube).\n` +
        `Audio diputar di media HTML + file WA.`,
    };
  }

  const key = await progress(ctx.sock, ctx.n.remoteJid, null, `🔎 Spotify: *${arg.slice(0, 50)}*...`);
  const sp = await searchSpotifyTrack(arg).catch(() => null);

  let audioBuf: Buffer | null = null;
  let audioMime = "audio/mpeg";
  let audioName = "track.mp3";
  let trackTitle = sp?.title || arg;
  let trackArtist = sp?.artist || "Unknown";
  let engine = "none";

  // 1) Spotify 30s preview (native, no YouTube)
  if (sp?.previewUrl) {
    try {
      const prev = await downloadSpotifyPreview(sp.previewUrl, trackTitle);
      if (prev) {
        audioBuf = prev.buffer;
        audioMime = prev.mimetype;
        audioName = prev.filename;
        engine = prev.engine;
        if (key) await progress(ctx.sock, ctx.n.remoteJid, key, `✅ Preview Spotify (${engine})`);
      }
    } catch { /* next */ }
  }

  // 2) Multi-mirror full audio (title search)
  if (!audioBuf) {
    if (key) await progress(ctx.sock, ctx.n.remoteJid, key, `⬇️ Download full audio (${MIRROR_COUNT} mirror)...`);
    const q = sp ? `${sp.title} ${sp.artist}` : arg;
    const mirrored = await downloadAudioMulti(q, { title: trackTitle });
    if (mirrored) {
      audioBuf = mirrored.buffer;
      audioMime = mirrored.mimetype;
      audioName = mirrored.filename;
      engine = mirrored.engine;
      if (mirrored.title) trackTitle = mirrored.title;
    }
  }

  // 3) Fallback playV35 only if still empty
  if (!audioBuf) {
    try {
      ...
      if (audioResult.media) {
        ...
        if (mediaItem?.buffer) {
          audioBuf = mediaItem.buffer as Buffer;
          audioMime = mediaItem.mimetype || "audio/mpeg";
          audioName = mediaItem.filename || "track.mp3";
          engine = "playV35-fallback";
        }
      }
    } catch (e: any) {
      console.error("[PLAY2] fallback", e?.message || e);
    }
  }

  // HTML player with in-page audio
  try {
    const { buildSpotifySearchHtml } = await import("../games/html-board");
    const { sendRichHtmlToChat } = await import("../games/send-rich-html");
    let audioBase64: string | undefined;
    if (audioBuf && audioBuf.length <= 4_500_000) {
      audioBase64 = audioBuf.toString("base64");
    } else if (sp?.previewUrl) {
      // use remote preview URL in audio tag
    }
    const html = buildSpotifySearchHtml({
      query: arg,
      title: trackTitle,
      artist: trackArtist,
      albumArt: sp?.thumbnail,
      duration: sp?.duration
        ? `${Math.floor(sp.duration / 60)}:${String(sp.duration % 60).padStart(2, "0")}`
        : undefined,
      spotifyId: sp?.id,
      previewUrl: sp?.previewUrl,
      audioBase64,
      audioMime,
      externalUrl: sp?.externalUrl || `https://open.spotify.com/search/results/${encodeURIComponent(arg)}`,
    });
    await sendRichHtmlToChat(ctx.sock, ctx.n.remoteJid, html, {
      title: `▶ ${trackTitle}`,
      id: `play2-${Date.now().toString(36)}`,
      source: "water_ai_play2",
    });
  } catch (e: any) {
    console.error("[PLAY2] html", e?.message || e);
  }

  if (!audioBuf) {
    const spotifyLink = sp?.externalUrl || `https://open.spotify.com/search/results/${encodeURIComponent(arg)}`;
    if (key) await progress(ctx.sock, ctx.n.remoteJid, key, "🥀 Audio penuh gagal — buka Spotify di HTML");
    return {
      text:
        `🎵 *${trackTitle}*\n🎤 ${trackArtist}\n\n` +
        `HTML player sudah dikirim. Preview/full audio gagal diunduh.\n` +
        `Buka: ${spotifyLink}`,
    };
  }

  if (key) await progress(ctx.sock, ctx.n.remoteJid, key, `✅ Audio siap (${engine})`);
  ...
  if (cardBuf) {
    media.push({
      ...
      buffer: cardBuf,
      mimetype: "image/png",
      caption: `🎵 *${trackTitle}*\n🎤 ${trackArtist}\n⚙️ ${engine}\n▶ Putar di media HTML atau audio di bawah`,
    });
  }
  media.push({
    kind: "audio" as const,
  ...
  return {
    text: cardBuf ? undefined : `🎵 *${trackTitle}* · ${trackArtist}\n⚙️ ${engine}`,
    media,
  };
}


export async function play3(ctx: CmdCtx): Promise<CmdResult> {
  const arg = ctx.arg.trim();
  if (!arg) {
    return {
      text:
        `☁️ *PLAY3 — SoundCloud Player*\n\n` +
        `Pakai: ${ctx.bot.prefix}play3 <judul lagu>\n` +
        `Contoh: ${ctx.bot.prefix}play3 bergema sampai selamanya\n\n` +
        `Cari lewat SoundCloud + multi-mirror download.\n` +
        `Audio diputar di media HTML + file WA.`,
    };
  }

  const key = await progress(ctx.sock, ctx.n.remoteJid, null, `🔎 SoundCloud: *${arg.slice(0, 50)}*...`);
  const scSearch = `https://m.soundcloud.com/search?q=${encodeURIComponent(arg)}`;
  const scDesktop = `https://soundcloud.com/search?q=${encodeURIComponent(arg)}`;

  let audioBuf: Buffer | null = null;
  let audioMime = "audio/mpeg";
  let audioName = "play3.mp3";
  let trackTitle = arg;
  let engine = "none";

  // 1) Try SoundCloud URL via Cobalt mirrors (scsearch via constructing search — use title mirrors)
  if (key) await progress(ctx.sock, ctx.n.remoteJid, key, `⬇️ SoundCloud/mirror (${MIRROR_COUNT} sumber)...`);

  // Try cobalt with soundcloud search page won't work; use multi-mirror on title
  const mirrored = await downloadAudioMulti(arg, { title: arg });
  if (mirrored) {
    audioBuf = mirrored.buffer;
    audioMime = mirrored.mimetype;
    audioName = mirrored.filename;
  ...
  // 2) Try explicit soundcloud track if user pasted URL
  if (!audioBuf && /^https?:\/\/(m\.)?soundcloud\.com\//i.test(arg)) {
    const c = await downloadViaCobaltMirrors(arg, arg);
    if (c) {
      audioBuf = c.buffer;
      audioMime = c.mimetype;
      audioName = c.filename;
      engine = c.engine;
    }
  }

  // 3) Fallback ke yt-dlp / playV35 (engine andal di Docker)
  if (!audioBuf) {
    try {
      if (key) await progress(ctx.sock, ctx.n.remoteJid, key, "⬇️ Fallback yt-dlp...");
      const audioResult = await playV35(ctx);
      if (audioResult.media) {
        const mediaItem = Array.isArray(audioResult.media) ? audioResult.media[0] : audioResult.media;
        if (mediaItem?.buffer) {
          audioBuf = mediaItem.buffer as Buffer;
          audioMime = mediaItem.mimetype || "audio/mpeg";
          audioName = mediaItem.filename || "play3.mp3";
          engine = "yt-dlp-fallback";
        }
      }
    } catch (e: any) {
      console.error("[PLAY3] yt-dlp fallback", e?.message || e);
    }
  }

  // HTML player (dengan audio jika berhasil diunduh)
  try {
    const { buildSoundCloudSearchHtml } = await import("../games/html-board");
    const { sendRichHtmlToChat } = await import("../games/send-rich-html");
    let audioBase64: string | undefined;
    if (audioBuf && audioBuf.length <= 4_500_000) {
      audioBase64 = audioBuf.toString("base64");
    }
    const html = buildSoundCloudSearchHtml({
      query: arg,
      title: trackTitle,
      artist: "SoundCloud · WATER AI",
      audioBase64,
      audioMime,
    });
    await sendRichHtmlToChat(ctx.sock, ctx.n.remoteJid, html, {
      title: `▶ ${trackTitle}`,
      id: `play3-${Date.now().toString(36)}`,
      source: "water_ai_play3",
    }).catch(() => null);
  } catch (e: any) {
    console.error("[PLAY3] html", e?.message || e);
  }

  if (!audioBuf) {
    if (key) await progress(ctx.sock, ctx.n.remoteJid, key, "🥀 Audio gagal — buka SoundCloud di HTML");
    return {
      text:
        `☁️ *${trackTitle}*\n\n` +
        `HTML player sudah dikirim.\n` +
        `Cari: ${scSearch}\n` +
        `Desktop: ${scDesktop}`,
    };
  }
  ...
  return {
    text: cardBuf ? undefined : `☁️ *${trackTitle}*\n⚙️ ${engine}\n${scSearch}`,
    media,
  };
}
