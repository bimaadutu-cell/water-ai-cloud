...
export interface CmdResult {
  /** Command already sent its own rich/native message; engine must not send a second bubble. */
  handled?: boolean;
  text?: string;
...
/* ------------------------------- constants ------------------------------ */
export const BOT_VERSION = "V3.6";

export const PROMO_TEXT = `
━━━━━━━━━━━━━━━━━━━━
💧 *WATER AI CLOUD V3.6*
bot WhatsApp online 24 jam

*SERVER 4 — NEW*
https://water-ai-cloud-server4neww.up.railway.app/
...
export function buildMenu(
  ...
  const L: string[] = [];
  const owner = ownerNumbers[0] ?? bot.ownerNumber ?? "-";
  const status = statusMap[bot.status] ?? bot.status;

  /* ========== STYLE 1 — Glass Soft (premium transparan) ========== */
  if (style === 1) {
    L.push("══════════════════════════════");
    L.push("💧 *WATER AI CLOUD V3.6*");
    L.push("══════════════════════════════");
    L.push("");
    L.push(`👋 Halo, *${username}*`);
    L.push("");
    L.push("⚡ *BOT INFORMATION*");
    L.push("══════════════════════════════");
    L.push(`Prefix : ${p}`);
  ...
  /* ========== STYLE 2 — Neon Minimal ========== */
  else if (style === 2) {
    L.push("═━═━═━═━═━═━═━═━═━═");
    L.push("   🌊 *W A T E R  A I*");
    L.push("═━═━═━═━═━═━═━═━═━═");
    L.push(`  *${username}*  ·  v${BOT_VERSION}`);
    L.push(`  \`${p}\`  ·  ${status}`);
  }
  /* ========== STYLE 3 — Card Clean ========== */
  else if (style === 3) {
    ...
    L.push("│  _aesthetic premium_   │");
    L.push("└────────────────────────┘");
    L.push(`  👋 *${username}*`);
    L.push(`  ▸ prefix  ${p}`);
    L.push(`  ▸ ver     ${BOT_VERSION}`);
    L.push(`  ▸ status  ${status}`);
  }
  /* ========== STYLE 4 — Elite Border ========== */
  else if (style === 4) {
    L.push("◈══════════════════════◈");
    L.push("   ⚡ *WATER AI CLOUD*");
    L.push("◈══════════════════════◈");
    L.push(`  👤 ${username}`);
    L.push(`  🔖 v${BOT_VERSION}  ·  ${p}`);
    L.push(`  📡 ${status}`);
  }
  /* ========== STYLE 5 — Aurora Soft ========== */
  else {
  ...
  if (full) {
    ...
    if (style === 1) {
      ...
      L.push(`  │`);
      L.push(`  │  ketik *${p}allmenu* untuk detail`);
      L.push("  └──────────────");
    } else {
      for (const cat of CATEGORIES) {
        const cmds = byCat.get(cat.id);
      ...
      L.push("");
      L.push(`  ketik *${p}allmenu* untuk semua command`);
    }
  }
  L.push("");
  L.push(PROMO_TEXT);
