...
async function aiChat(
  ...
  const preferredModel =
    ...
    (isGeminiKey ? "gemini-2.0-flash" : isGroq ? "llama-3.3-70b-versatile" : "gpt-4o-mini");

  // Fallback chain — older/invalid model names cause continuous 404 "reject endpoint"
  const modelCandidates: string[] = isGeminiKey
    ? [
        preferredModel,
        "gemini-2.0-flash",
    ...
    : isGroq
      ? [preferredModel, "llama-3.3-70b-versatile", "llama-3.1-70b-versatile", "gemma2-9b-it"]
      : [preferredModel, "gpt-4o-mini", "gpt-4o", "gpt-3.5-turbo"];

  const uniqueModels = [...new Set(modelCandidates.map((m) => m.replace(/^models\//, "").trim()).filter(Boolean))];

  const messages: any[] = [{ role: "system", content: system }];
  if (opts.history?.length) {
    for (const h of opts.history.slice(-40)) {
    ...
    for (const base of bases) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          errors.push(`${url} [${model}]: empty response`);
        } else {
          const errBody = await res.text().catch(() => "");
          errors.push(`${url} [${model}] → HTTP ${res.status} ${errBody.slice(0, 80)}`);
          // 404 model not found → try next model; 401/403 → next base
          if (res.status === 401 || res.status === 403) continue;
        }
      } catch (e: any) {
        errors.push(`${base}/chat/completions [${model}]: ${String(e?.message || e).slice(0, 80)}`);
      }

      // 2) Gemini native generateContent
      if (isGeminiKey && /generativelanguage\.googleapis\.com/i.test(base)) {
        try {
          ...
          if (res.ok) {
            ...
            const errBody = await res.text().catch(() => "");
            errors.push(`native ${nativeModel} → HTTP ${res.status} ${errBody.slice(0, 80)}`);
          }
        } catch (e: any) {
          errors.push(`native ${model}: ${String(e?.message || e).slice(0, 80)}`);
        }
      }
    }
  }
...
const WA_SYSTEM =
  ...
  "Analisis mendalam, akurat, jujur, dan ingat konteks percakapan. " +
  "Jawab dalam bahasa user. Gunakan penalaran step-by-step bila diminta. " +
  "Hindari markdown berat; emoji secukupnya.";

export async function ai(ctx: CmdCtx): Promise<CmdResult> {
  if (!ctx.arg) return { text: "Pakai: .ai <pertanyaan>\n`.ai clear` hapus memori chat ini." };
  if (/^(clear|reset|lupa)$/i.test(ctx.arg.trim())) {
    const { clearChatMemory } = await import("./state");
    clearChatMemory(ctx.bot.id, ctx.n.remoteJid, ctx.n.sender);
    return { text: "🧹 Memori AI untuk chat ini dihapus." };
  }
  const { getChatMemory, pushChatMemory } = await import("./state");
  const history = getChatMemory(ctx.bot.id, ctx.n.remoteJid, ctx.n.sender).map((m) => ({
    role: m.role,
    content: m.content,
  }));
  pushChatMemory(ctx.bot.id, ctx.n.remoteJid, "user", ctx.arg, ctx.n.sender);
  const r = await aiChat(WA_SYSTEM, ctx.arg, {
    botSettings: ctx.bot.settings,
    history,
    maxTokens: 8192,
  });
  pushChatMemory(ctx.bot.id, ctx.n.remoteJid, "assistant", r, ctx.n.sender);
  return { text: truncate(r, 4000) };
}
export const ask = ai;
